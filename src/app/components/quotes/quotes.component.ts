...
export class QuotesComponent implements OnInit {
  ...
  quotes: Quote[] = [
    ...
    new Quote(
      'When one door closes, another opens, but we often look so long at the closed door that we do not see the one that has been opened for us',
      'Hellen Keller',
      'John',new Date(2022,3,11)
    ),
  ];
  ...
  upVote(index: number) {
    ...
    for (let i = 0; i < this.quotes.length; i++) {
      if (this.quotes[i].upVote > this.highestQuote.upVote) {
        this.highestQuote = this.quotes[i];
      }
    }
  }
